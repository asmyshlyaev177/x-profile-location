...
│   │   ├── Footer.tsx
│   │   ├── InstallButton.tsx # Browser-detected install link, `signal` / `void` tones; reports `install_click`
...
│   ├── utils/
│   │   ├── analytics.ts      # trackEvent() — the only caller of window.gtag
│   │   ├── browser.ts        # Browser detection for install links
│   │   └── constants.ts      # Store, repo and donate URLs
...
- **Analytics only loads when there is an ID to send to.** `gtag` is ~87 kB,
  ...
  The only event the site sends is `install_click`, from `<InstallButton>`, with
  `placement` (which fold), `browser` and `store_url`. Everything goes through
  `trackEvent()` in `utils/analytics.ts`, which no-ops during SSR and in any
  build without an ID — so components never guard on `import.meta.env.PROD`.
  Two idle waits sit in front of it: hydration attaches the handler, gtag.js
  drains the queue. A click inside either window navigates to the store
  uncounted, which is a deliberate trade against delaying the install for
  `event_callback`. Copy in `Trust.tsx` names what is collected — keep it
  honest if a second event is ever added.
