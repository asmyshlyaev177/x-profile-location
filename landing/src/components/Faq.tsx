...
export function Faq({
  ...
  return (
    <section id={id} class="bg-surface hairline band relative scroll-mt-24">
      ...
      <div class="shell-narrow max-w-3xl!">
        ...
        <div class="border-line mt-12 border-t">
          {items.map((item, i) => (
            <details
              key={item.q}
              class="border-line reveal group border-b"
            ...
            >
              ...
              <summary class="flex cursor-pointer list-none items-start gap-4 py-5 [&::-webkit-details-marker]:hidden">
                <h3 class="text-ink flex-1 text-[1.0625rem] font-semibold">
                  {item.q}
                </h3>
                {/* A plus that becomes a minus. Two rules beat an icon font. */}
                <span
                  aria-hidden="true"
                  class="text-accent relative mt-1 h-4 w-4 shrink-0"
                >
                  <span class="bg-accent absolute top-1/2 left-0 h-px w-4 -translate-y-1/2" />
                  <span class="bg-accent absolute top-0 left-1/2 h-4 w-px -translate-x-1/2 transition-transform duration-200 group-open:scale-y-0" />
                </span>
              </summary>
