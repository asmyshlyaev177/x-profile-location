...
import { detectBrowser, STORES } from '../utils/browser'
...
export function InstallButton({ size = 'lg', override }: InstallButtonProps) {
  const [browser, setBrowser] = useState<SupportedBrowser>('chrome')

  useEffect(() => {
    setBrowser(override ?? detectBrowser())
  }, [override])

  const store = STORES[browser]
  const px = size === 'lg' ? 'px-8 py-3.5' : 'px-6 py-3'

  return (
    <a
      href={store.url}
      class={`inline-flex items-center gap-2 rounded-full bg-white ${px} text-sm font-bold text-black transition-all duration-200 hover:bg-white/90 hover:shadow-[0_0_20px_rgba(255,255,255,0.15)]`}
    >
