...
async function generate(srcPath, destPath, width, height, { noAlpha = false } = {}) {
  ...
  console.log(`✓ ${destPath.replace(landingDir + '/', '')} (${width}×${height})`)
}

await generate(join(publicDir, 'og-image.svg'),      join(publicDir, 'og-image.png'),                 1200, 630)
await generate(join(publicDir, 'favicon.svg'),       join(publicDir, 'apple-touch-icon.png'),          180, 180)
await generate(join(publicDir, 'promo-small.svg'),   join(storeDir,  'promo-small.png'),               440, 280, { noAlpha: true })
await generate(join(publicDir, 'promo-marquee.svg'), join(storeDir,  'promo-marquee.png'),            1400, 560, { noAlpha: true })
