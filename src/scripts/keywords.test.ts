...
describe('graphemeIncludesWord', () => {
  it('matches keyword as a whole word', () => {
    expect(graphemeIncludesWord(toGraphemes('i love nft art'), toGraphemes('nft'))).toBe(true)
  })

  it('does not match keyword inside a longer word', () => {
    expect(graphemeIncludesWord(toGraphemes('nftart'), toGraphemes('nft'))).toBe(false)
    expect(graphemeIncludesWord(toGraphemes('mynft'), toGraphemes('nft'))).toBe(false)
    expect(graphemeIncludesWord(toGraphemes('mynftart'), toGraphemes('nft'))).toBe(false)
  })

  it('matches keyword adjacent to symbols like # and $', () => {
    expect(graphemeIncludesWord(toGraphemes('#nft'), toGraphemes('nft'))).toBe(true)
    expect(graphemeIncludesWord(toGraphemes('$nft'), toGraphemes('nft'))).toBe(true)
    expect(graphemeIncludesWord(toGraphemes('nft!'), toGraphemes('nft'))).toBe(true)
    expect(graphemeIncludesWord(toGraphemes('loves #nft trading'), toGraphemes('nft'))).toBe(true)
  })

  it('matches keyword at start and end of string', () => {
    expect(graphemeIncludesWord(toGraphemes('nft'), toGraphemes('nft'))).toBe(true)
    expect(graphemeIncludesWord(toGraphemes('nft trader'), toGraphemes('nft'))).toBe(true)
    expect(graphemeIncludesWord(toGraphemes('love nft'), toGraphemes('nft'))).toBe(true)
  })

  it('does not match keyword when preceded by a digit', () => {
    expect(graphemeIncludesWord(toGraphemes('1nft'), toGraphemes('nft'))).toBe(false)
  })
  ...
  it('matches non-Latin keyword as a whole word (Cyrillic)', () => {
    expect(graphemeIncludesWord(toGraphemes('люблю крипто'), toGraphemes('крипто'))).toBe(true)
    expect(graphemeIncludesWord(toGraphemes('крипто'), toGraphemes('крипто'))).toBe(true)
  })
  ...
  it('does not match non-Latin keyword inside a longer word (Cyrillic)', () => {
    ...
    expect(graphemeIncludesWord(toGraphemes('крипторынок'), toGraphemes('крипто'))).toBe(false)
    expect(graphemeIncludesWord(toGraphemes('некрипто'), toGraphemes('крипто'))).toBe(false)
  })
