...
describe('fetch — header capture', () => {
  ...
  it('does NOT capture headers when authorization is missing', async () => {
    await import('./page-script');
    let fired = false;
    window.addEventListener('x-loc-headers-captured', () => { fired = true; }, { once: true });
  ...
  it('only captures headers once — subsequent graphql requests do not re-dispatch', async () => {
    await import('./page-script');
    let count = 0;
    window.addEventListener('x-loc-headers-captured', () => { count++; });
...
describe('fetch — bio extraction', () => {
  it('dispatches x-loc-users-data for HomeTimeline response', async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify(homeTimelineResponse('tweetuser', 'my bio')), {
          status: 200,
        }),
      ),
    );
  ...
  it('dispatches x-loc-users-data for TweetDetail response', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify(tweetDetailResponse('detailuser', 'detail bio')), {
          status: 200,
        }),
      ),
    );
  ...
  it('does NOT dispatch x-loc-users-data for non-intercepted query names', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify(homeTimelineResponse('skip', 'bio')), { status: 200 }),
      ),
    );

    await import('./page-script');
    let fired = false;
    window.addEventListener('x-loc-users-data', () => { fired = true; }, { once: true });
  ...
  it('does NOT dispatch x-loc-users-data when response contains no User nodes', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ data: { no_users_here: true } }), { status: 200 }),
      ),
    );
...
describe('fetch — user deduplication', () => {
  it('deduplicates users by userName (case-insensitive), keeping first occurrence', async () => {
    const payload = {
      data: [
        { __typename: 'User', core: { screen_name: 'DupeUser' }, legacy: { description: 'first' } },
        { __typename: 'User', core: { screen_name: 'dupeuser' }, legacy: { description: 'second' } },
        { __typename: 'User', core: { screen_name: 'UniqueUser' }, legacy: { description: 'unique' } },
      ],
    };
    ...
    const names = ev.detail.users.map((u: { userName: string }) => u.userName.toLowerCase());
...
describe('x-loc-request-headers event', () => {
  it('re-emits stored headers when x-loc-request-headers is dispatched after capture', async () => {
    ...
    expect(
      emitted.some((e) => e.detail?.headers?.authorization === 'Bearer stored123'),
    ).toBe(true);
  ...
  it('can re-emit stored headers multiple times on repeated x-loc-request-headers events', async () => {
    ...
    const emitted = spy.mock.calls
      .map(([e]) => e as CustomEvent)
      .filter((e) => e.type === 'x-loc-headers-captured' && e.detail?.headers?.authorization === 'Bearer multiemit');
...
describe('XHR — header capture', () => {
  it('dispatches x-loc-headers-captured when graphql XHR has authorization header', async () => {
    ...
    const xhr = new (window.XMLHttpRequest as unknown as new () => XMLHttpRequest)();
...
describe('XHR — bio extraction', () => {
  it('dispatches x-loc-users-data from HomeTimeline XHR load event', async () => {
    ...
    (xhr as unknown as FakeXHR & { responseText: string }).responseText = JSON.stringify(
      homeTimelineResponse('xhruser', 'xhr bio'),
    );
  ...
  it('dispatches x-loc-users-data from TweetDetail XHR load event', async () => {
    ...
    const xhr = new (window.XMLHttpRequest as unknown as new () => XMLHttpRequest)();
    (xhr as unknown as FakeXHR & { responseText: string }).responseText = JSON.stringify(
      tweetDetailResponse('xhrdetail', 'xhr detail bio'),
    );
